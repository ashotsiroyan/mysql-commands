            ...
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    for (let i = 0; i < keys.length; ++i) {
                        let model = connection.models[keys[i]], { columns, indexes } = model.schema.query;
                        ...
                        yield Singleton.execute(`ALTER TABLE ${model.modelName} ADD IF NOT EXISTS (${columns.join(', ')})${indexes.length > 0 ? `, ADD INDEX IF NOT EXISTS ${indexes.join(', ADD INDEX IF NOT EXISTS ')}` : ''}, MODIFY IF EXISTS ${columns.join(', MODIFY IF EXISTS ')};`);
                    }
                    return connection;
                }
                catch (err) {
        ...
        createConnection: function (props) {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    for (let i = 0; i < keys.length; ++i) {
                        let model = _conn.models[keys[i]], { columns, indexes } = model.schema.query;
                        yield Singleton.execute(`CREATE TABLE IF NOT EXISTS ${model.modelName} (${columns.join(', ')}${indexes.length > 0 ? `, INDEX ${indexes.join(', INDEX ')}` : ''});`);
                        yield Singleton.execute(`ALTER TABLE ${model.modelName} ADD IF NOT EXISTS (${columns.join(', ')})${indexes.length > 0 ? `, ADD INDEX IF NOT EXISTS ${indexes.join(', ADD INDEX IF NOT EXISTS ')}` : ''}, MODIFY IF EXISTS ${columns.join(', MODIFY IF EXISTS ')};`);
                    }

            ...
            const saveNext = () => {
                ...
                keys.forEach((key) => {
                    ...
                    if (!this.isNew && __classPrivateFieldGet(this, _schema).options.timestamps && key === '_updatedAt')
                        value = new Date();
                    if (value) {
                        value = mysql_1.default.escape(WithOptions(value, __classPrivateFieldGet(this, _schema).obj[key]));
                        if (this.isNew) {
                            cols += `${key}, `;
                            values += `${value}, `;
                        }
    ...
    update(doc = {}, callback) {
        try {
            if (Object.keys(doc).length > 0) {
                ...
                const updateNext = () => {
                    let keys = Object.keys(__classPrivateFieldGet(this, _schema).obj), updateString = "";
                    keys.forEach((key) => {
                        if (doc[key])
                            this[key] = doc[key];
                        let value = this[key];
                        if (__classPrivateFieldGet(this, _schema).options.timestamps && key === '_updatedAt')
                            value = new Date();
                        if (value) {
                            value = mysql_1.default.escape(WithOptions(value, __classPrivateFieldGet(this, _schema).obj[key]));
                            if (key !== '_id') {
    ...
    convertData({ doc }) {
        ...
        keys.forEach((key) => {
            if (this.isNew) {
                ...
                if (doc[key])
                    value = WithOptions(doc[key], __classPrivateFieldGet(this, _schema).obj[key]);
                else if (defaultValue)
                    value = WithOptions(defaultValue, __classPrivateFieldGet(this, _schema).obj[key]);
                if (hasId && key === '_id')
